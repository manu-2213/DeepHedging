...
class HedgingEnv(VectorEnv):
    def __init__(
        ...
        - data_generation: Can be set to GBM or Heston
        """
        ...
        self.K_expanded = self.K[None, :, :].repeat(
    ...
    def _generate_simulations(self):
        ...
        if self.data_generation == "gbm":
        ...
        elif self.data_generation == "heston":
            kappa = 2.0  # Standard mean reversion speed
            theta = self.sigma**2  # Long-term variance set to match GBM volatility
            xi = 0.3 * self.sigma  # Volatility of volatility as fraction of volatility
            rho = -0.7  # Typical negative correlation (leverage effect)
            v0 = self.sigma**2  # Initial variance set to current volatility level

            v = np.zeros((self.num_simulation, self.num_asset, self.num_step + 1))
            v[:, :, 0] = v0
    
            z_s = z  # Already generated normal random variables for asset price
    
            # Generate second set of random variables for variance process
            z_v = self.np_random.normal(0, 1, (self.num_simulation, self.num_asset, self.num_step))
    
            # Apply correlation between price and volatility shocks
            z_v = rho * z_s + np.sqrt(1 - rho**2) * z_v
    
            # Initialize arrays for returns
            log_returns = np.zeros((self.num_simulation, self.num_asset, self.num_step))
            ...
            for t in range(self.num_step):
                ...
                v[:, :, t+1] = v[:, :, t] + kappa * (theta - v_t) * self.dt + \
                            xi * np.sqrt(v_t * self.dt) * z_v[:, :, t]
                
                # Calculate log returns using the current variance
                log_returns[:, :, t] = (self.r - 0.5 * v_t) * self.dt + \
                                    np.sqrt(v_t * self.dt) * z_s[:, :, t]
    ...
    def _create_observations(self):
        ...
        moneyness = self.K_expanded / S_expanded
