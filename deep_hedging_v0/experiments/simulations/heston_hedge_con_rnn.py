...
from hedging.envs import HedgeConcHeston
...
def main():
    ...
    base_env = HedgeConcHeston(
        S0=S0, K=K, P=P, r=r, v0=v0, theta=params["theta"], rho=params["rho"],
    ...
    model = create_ppo_rnn_actor_exotic(
        input_dim=input_dim, action_dim=action_dim, hidden_dim=hidden_size
    )
    model.to(device)
    ...
    model, _ = action_training(
        env, model, num_epochs, num_episodes, device,
        advantage_module, loss_module, optim,
        frames_per_batch, sub_batch_num, sub_batch_size,
        log_frquency=1,
    )
