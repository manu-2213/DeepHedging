...
def main():
    ...
    cfg = get_heston_conc_config()
    cfg["history_len"] = 10
    training_cfg = TrainingConfig(num_epochs=20, num_episodes=200)
    ppo_cfg = PPOConfig(entropy_coeff=1e-3, learning_rate=1e-4)

    def build_env(num_paths: int) -> HedgeConcHeston:
        return HedgeConcHeston(
            S0=cfg["S0"],
            K=cfg["K"],
            P=cfg["P"],
            r=cfg["r"],
