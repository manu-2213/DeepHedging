...
def main():
    ...
    cfg = get_heston_call_config()
    cfg["num_steps"] = 25
    cfg["history_len"] = 5
    training_cfg = TrainingConfig(num_epochs=5, num_episodes=50)
    ppo_cfg = PPOConfig(entropy_coeff=0.0, learning_rate=1e-4)

    def build_env(num_paths: int) -> HedgeCallHeston:
    ...
    sub_batch_size = max(1, frames_per_batch // sub_batch_num)

    model = create_ppo_rnn_actor(
    ...
    test_env = build_env(num_paths=6)
