...
from hedging.feature_extractor import create_observation_hedge_conc


class HedgeConc(HedgeBase):
    def __init__(
        self,
        S0,
        K,
    ...
    def post_reset(self):
        ...
        self.option_prices = self.simulator.cash_or_nothing_call(self.K, self.P)  # CoN prices
    ...
    def _create_observations(self):
        return create_observation_hedge_conc(
            self.current_step,
            self.stock_prices,
            self.option_prices,
            self.call_prices,
